...
func TestLoadHappyPath(t *testing.T) {
	runCases(t, []configCase{
		...
		},
		{
			...
			check: func(t *testing.T, cfg Config) {
				if cfg["docs"] != "docs/" || cfg["output"] != "docs/_build/" {
	...
	runCases(t, []configCase{
		{
			...
			data: base(map[string]any{"posts": map[string]any{"dir": ".selfdoc/posts/"}}),
			check: func(t *testing.T, cfg Config) {
				if cfg["posts"].(map[string]any)["dir"] != ".selfdoc/posts/" {
