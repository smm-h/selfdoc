...
// writeManifest writes the checkout's .selfdoc/manifest.json, the way gen does.
//
...
func writeHomeCheckout(t *testing.T, root string) string {
	...
	writeBytes(t, filepath.Join(root, "docs", "assets", "photo.png"), png2x2)

	// The curated listing the front page and /projects/ both render from.
	writeText(t, filepath.Join(root, "docs", "projects.toml"), listingTOML)
...
func buildStandaloneProject(t *testing.T, root, theme string) string {
	...
	out := filepath.Join(checkout, "docs", "_build")
