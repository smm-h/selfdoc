...
func TestVER004GeneratedRootFileVersion(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			projectConfig := pythonProjectConfig()
			projectConfig["root_files"] = []any{"docs/_README.md"}
			root := versionProject(t, projectConfig, "1.0.0")
			write(t, filepath.Join(root, "docs", "_README.md"), testCase.template)
...
func multiVersionProject(t *testing.T) string {
	...
	write(t, filepath.Join(root, "docs", "index.md"), "# Project\n\nWelcome.\n")
	runGit(t, root, "add", "docs/")
