...
func cvProject(t *testing.T, document string) string {
	...
	write(t, filepath.Join(root, "docs", "cv.toml"), document)
	return root
}
...
func TestAMisspellingInTheCVDocumentIsReported(t *testing.T) {
	...
	if matching[0].File() != "docs/cv.toml" {
		t.Errorf("file = %q, want docs/cv.toml", matching[0].File())
	}
	raw, err := os.ReadFile(filepath.Join(root, "docs", "cv.toml"))
	if err != nil {
...
func TestACopyInTheBuildOutputIsNotReported(t *testing.T) {
	...
	// anyone can fix.
	write(t, filepath.Join(root, "docs", "_build", "cv.toml"), document)
	...
	if len(files) != 1 || files[0] != "docs/cv.toml" {
		t.Errorf("files = %v, want only docs/cv.toml", files)
	}
}
...
func TestThePagesOwnProseIsNotReportedTwice(t *testing.T) {
	...
	write(t, filepath.Join(root, "docs", "cv.toml"), cvDocument)
