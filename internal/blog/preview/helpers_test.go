...
func writeCheckout(t *testing.T, spec checkoutSpec) string {
	...
	testproject.WriteJSON(t, filepath.Join(spec.Root, ".selfdoc", "manifest.json"),
		fixtureManifest(spec.Slug, spec.Name, spec.Version, spec.Pages, spec.Posts))

	buildDir := filepath.Join(spec.Root, "docs", "_build")
	...
	if spec.Listing != "" {
		testproject.WriteText(t, filepath.Join(spec.Root, "docs", "projects.toml"), spec.Listing)
	}
	return spec.Root
}
