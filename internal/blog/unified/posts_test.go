...
func writePost(t *testing.T, projectDir, name, source string) {
	t.Helper()
	testproject.WriteText(t, filepath.Join(projectDir, ".selfdoc", "posts", name), source)
}
...
func TestBuildUnifiedCleansUpTheInjectedPostsWhenTheBuildFails(t *testing.T) {
	...
	assertNoInjectedPosts(t, filepath.Join(coreDir, "docs"))
	assertNoInjectedPosts(t, filepath.Join(cliDir, "docs"))
	assertNoInjectedPosts(t, filepath.Join(docsSite, "docs"))
}
...
func TestBuildUnifiedLeavesADocsTreeAloneWhenThereAreNoPosts(t *testing.T) {
	...
	assertNoInjectedPosts(t, filepath.Join(projectDirOf(docsSite, "core"), "docs"))
	assertNoInjectedPosts(t, filepath.Join(docsSite, "docs"))
