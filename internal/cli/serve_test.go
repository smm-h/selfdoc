...
func servedProject(t *testing.T) string {
	t.Helper()
	dir := postProject(t, map[string]any{"docs": "docs/", "output": "docs/_build/"})
...
func TestServeRefusesWithoutAnOutputTree(t *testing.T) {
	...
	if !strings.Contains(result.Stderr, "Output directory 'docs/_build' not found") {
...
func TestServeWithDraftsRebuildsFirst(t *testing.T) {
	...
	writeText(t, filepath.Join(dir, "docs", "_build", "placeholder.txt"), "placeholder\n")
...
func TestServeWithoutDraftsDoesNotRebuild(t *testing.T) {
	isolate(t)
	dir := servedProject(t)
	sentinel := filepath.Join(dir, "docs", "_build", "index.html")
