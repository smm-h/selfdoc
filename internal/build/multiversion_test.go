...
func buildVersioned(t *testing.T, versions []string, apply func(*Options)) site {
	...
	return site{
		dir:     dir,
		output:  filepath.Join(dir, "docs", "_build"),
		written: written,
	}
}
...
func TestBuildMultipleVersions(t *testing.T) {
	...
	t.Run("each extracted version has a cache entry, and the cache is ignored", func(t *testing.T) {
		cacheDir := filepath.Join(built.dir, ".selfdoc", "cache")
		if !isDir(cacheDir) {
		...
		gitignore := readFile(t, filepath.Join(cacheDir, ".gitignore"))
		if !strings.Contains(gitignore, "*") {
			t.Errorf("the cache's .gitignore is %q, want one ignoring everything", gitignore)
		}
	})
...
func TestExtractVersionContent(t *testing.T) {
	...
	t.Run("the tag's docs are extracted into the cache", func(t *testing.T) {
		...
		content := readFile(t, filepath.Join(cacheDir, "docs", "index.md"))
