...
func TestBuildRefusesAnAuthoredReservedPage(t *testing.T) {
	...
	t.Run("a project with posts and no authored reserved page is unaffected", func(t *testing.T) {
		...
		if isFile(filepath.Join(built.dir, "docs", "blog.md")) {
			t.Error("the injected listing page was left in the docs tree")
		}
		if isDir(filepath.Join(built.dir, "docs", "blog")) {
...
func TestCheckReservedAuthoredPages(t *testing.T) {
	...
	t.Run("a clean docs tree passes", func(t *testing.T) {
		dir := t.TempDir()
		testproject.WriteText(t, filepath.Join(dir, "docs", "index.md"), "# Home\n")
		if err := CheckReservedAuthoredPages(filepath.Join(dir, "docs"), dir); err != nil {
	...
	t.Run("the reported path is relative to the base directory", func(t *testing.T) {
		dir := t.TempDir()
		testproject.WriteText(t, filepath.Join(dir, "docs", "blog.md"), authoredBlogPage)
		err := CheckReservedAuthoredPages(filepath.Join(dir, "docs"), dir)
		if err == nil {
			t.Fatal("an authored page on a reserved path was accepted")
		}
		assertCarries(t, "the refusal", err.Error(), "'docs/blog.md'")
	})
}
