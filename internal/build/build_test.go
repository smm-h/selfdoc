...
func tryBuildFixture(t *testing.T, f fixture) (site, error) {
	...
	for relPath, content := range f.Docs {
		testproject.WriteText(t, filepath.Join(dir, "docs", filepath.FromSlash(relPath)), content)
	}
...
func TestBuildRefusals(t *testing.T) {
	...
	t.Run("a config with no author", func(t *testing.T) {
		dir := testproject.Make(t, map[string]any{"docs": "docs/", "output": "docs/_build/"})
		raw := testproject.DefaultConfig(map[string]any{"docs": "docs/", "output": "docs/_build/"})
		delete(raw, "author")
