...
func makeAddressFixture(t *testing.T, configExtra map[string]any, localeCodes []string) string {
	...
	for _, code := range localeCodes {
		if localized {
			docRoots = append(docRoots, filepath.Join(dir, "docs", code))
			continue
		}
		docRoots = append(docRoots, filepath.Join(dir, "docs"))
	}
	if localized {
		...
		if err := os.Remove(filepath.Join(dir, "docs", "index.md")); err != nil {
	...
	for _, version := range versions {
		...
		testproject.Git(t, dir, "add", "docs")
...
func buildAddressFixture(t *testing.T, configExtra map[string]any, localeCodes []string) site {
	...
	return site{dir: dir, output: filepath.Join(dir, "docs", "_build"), written: written}
}
...
func TestABuildThatProducesNoContentPagesIsAnError(t *testing.T) {
	...
	_, err = buildBody(bodyInputs{
		...
		outputDir:         filepath.Join(dir, "docs", "_build"),
		docsDirName:       "docs",
		partitions:        blind,
