...
| Every project is reachable | A declared project whose index page neither the site root nor the `/projects/` listing links, so it is published where no reader arriving at the site and no crawler following links ever gets to it. Either page may curate what it shows. The home project is exempt: the site root is its own front page |
