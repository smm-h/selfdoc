...
def generate_worker_js(
    ...
    * ``/<slug>/posts/<post>/`` -> ``/blog/<post>/``.  Posts moved out of
    ...
    return (
        ...
        "\n"
        "// One pass of the mapping: the path this address maps to, or null.\n"
        "function legacyStep(pathname) {\n"
        ...
        "// The mapping is applied to its own output until it stops matching:\n"
        "// /<slug>/<locale>/<version>/<slug>/<locale>/<version>/x maps to an\n"
        "// address that is itself historical, and a redirect that lands on a\n"
        "// redirect is a chain. Every pass drops at least one path segment,\n"
        "// so the segment count bounds the number of passes and a path built\n"
        "// to loop terminates.\n"
        "export function legacyTarget(pathname) {\n"
        "  let current = pathname;\n"
        "  let target = null;\n"
        "  for (let left = pathname.split(\"/\").length; left > 0; left--) {\n"
        "    const next = legacyStep(current);\n"
