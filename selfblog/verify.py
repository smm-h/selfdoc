...
def _not_found_failures(tree: AssemblyTree) -> list[Failure]:
    ...
    # The 404 sits at the site root, so the way back to each of the three
    # is the bare address -- written relative like every other link a
    # reader clicks, so it still works on a preview or a mirror.
    for rel, href in (
        ("/", "./"),
        ("/projects/", "projects/"),
        (f"/{POSTS_SEGMENT}/", f"{POSTS_SEGMENT}/"),
    ):
        if f'href="{href}"' not in not_found:
            failures.append(Failure(
                "shared-artifacts", "site/404.html",
...
def extract_link_registry(tree: AssemblyTree) -> dict[str, list[str]]:
    ...
    Site-level pages are not read at all.  A post has no project segment,
    so its first path segment is ``blog`` -- not a project, and treating
    it as one makes every link a post's own chrome writes back into the
    project that published it look like a cross-project link.  Those links
    are generated by that project's build from its own addresses, and they
    reach pages the manifests never list (the glossary, the API and CLI
    indexes are published without appearing there).  Whether they resolve
    is the LINK001 pass's question, and it answers it for every one.
    """
    registry: dict[str, list[str]] = {}
    for page_rel in tree.pages:
        if is_site_level(page_rel):
            continue
