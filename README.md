...
| `layout dump` | Print selfdoc's layout declaration: every directory it claims, whether the directory is handwritten or generated, whether the repository commits it, and the paths it replaced |
| `layout validate` | Check this repository's .stricttools/ directory: every directory is named in OWNERS.csv and everything named exists, every directory selfdoc owns holds only what its side allows, nothing inside starts with a dot except the derived ignore file, and that ignore file is what selfdoc's declaration renders |
