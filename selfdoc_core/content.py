...
def resolve_table_directives() -> str:
    ...
    from selfdoc_core.catalog import CORE_DIRECTIVES
...
def resolve_content(
    ...
    *, config: dict | None = None,
    ...
    After checking built-in directives, consults the directive registry
    for externally-registered resolvers (e.g. table-commands from selfdoc).
    """
    ...
    registry = get_directive_registry()
    if name in registry:
        resolver_fn = registry[name]
        return resolver_fn(name, attrs, body, base_dir, config=config)
